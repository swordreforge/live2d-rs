...
bool LAppModel::loadModelJson(const std::string& path) {
    ...
    // Apply layout (match Python L2DModelMatrix.layout)
    auto layoutPos = json.find("\"layout\"");
    if (layoutPos != std::string::npos) {
        auto lo = json.find('{', layoutPos);
        auto lc = json.find('}', lo);
        if (lo != std::string::npos && lc != std::string::npos) {
            std::string layout = json.substr(lo + 1, lc - lo - 1);
            auto findVal = [&](const char* key) -> float* {
                std::string quoted = std::string("\"") + key + "\"";
                auto kp = layout.find(quoted);
                if (kp == std::string::npos) return nullptr;
                auto col = layout.find(':', kp);
                if (col == std::string::npos) return nullptr;
                auto* v = new float((float)std::atof(layout.c_str() + col + 1));
                return v;
            };
            #define APPLY_LAYOUT(key, method) do { \
                auto* v = findVal(key); \
                if (v) { mModelMatrix.method(*v); delete v; } \
            } while(0)
            APPLY_LAYOUT("width", setWidth);
            APPLY_LAYOUT("height", setHeight);
            APPLY_LAYOUT("x", translateX);
            APPLY_LAYOUT("y", translateY);
            APPLY_LAYOUT("center_x", centerX);
            APPLY_LAYOUT("center_y", centerY);
            APPLY_LAYOUT("top", top);
            APPLY_LAYOUT("bottom", bottom);
            APPLY_LAYOUT("left", left);
            APPLY_LAYOUT("right", right);
            #undef APPLY_LAYOUT
        }
    }
